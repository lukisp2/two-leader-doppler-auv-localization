# Archived v1.0.0 qualification figure

This document describes the two-panel Figure 9 shipped with `v1.0.0`. The
post-review manuscript replaces it with a three-panel diagnosis-and-repair
figure generated by `code/make_publication_figure_policy_execution_current.py`
from `data/tables/v40_publication_results.json` and
`data/tables/controller_repair_results.json`. The replacement does not read a
private campaign path; see `docs/TABLES_AND_DATA.md`.
